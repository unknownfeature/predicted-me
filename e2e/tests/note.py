...
get_note_supplier = lambda jwt: lambda: data.get(query_params={constants.note_id: id}, jwt=jwt)[0]
...
@pytest.mark.asyncio
async def test_data_extracted_from_note_text(jwt: str, attempts=3):
    text = os.getenv('TEST_TEXT')
    id = note.create(text=text, jwt=jwt)
    stored_note = note.get(id, jwt=jwt)[0]
...
@pytest.mark.asyncio
async def test_data_extracted_from_audio(jwt: str, attempts=3):
    source_audio_key = os.getenv('SOURCE_AUDIO_KEY')
...
@pytest.mark.asyncio
async def test_data_extracted_from_image( jwt: str, attempts=3):
    source_image_key = os.getenv('SOURCE_IMAGE_KEY')
