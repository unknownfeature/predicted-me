...
from backend.lib.db import DataSchedule, Metric, TaskSchedule, Task
...
updatable_fields = {'recurrence_schedule', 'priority'}

patch_handler = lambda session, update_fields, user_id, id: session.execute(
    update(TaskSchedule).where(TaskSchedule.task_id == Task.id).where(
        and_([TaskSchedule.id == id, Task.user_id == user_id])).values(**update_fields))

delete_handler = lambda session, user_id, id: session.execute(
    sql_delete(TaskSchedule).where(TaskSchedule.task_id == Task.id).where(
        and_([TaskSchedule.id == id, Task.user_id == user_id])))

post_handler = lambda context, _: DataSchedule(**{f: context.body[f] for f in context.body if f in updatable_fields} | {
    'task_id': context.path_params['id'], 'user_id': context.user.id})
